...
db.collection("users").doc(workerId).get()
    .then((doc) => {
        if (doc.exists) {
            ...
            document.getElementById('workerphoneNumber').textContent = `رقم الهاتف: ${workerData.phoneNumber}`;
            document.getElementById('workernewPhone').textContent = `رقم الهاتف: ${workerData.newphone}`;
